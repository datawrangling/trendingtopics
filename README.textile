...
A quick look at lib/python_streaming/daily_timelines.py shows the filters we applied to page urls
...
The first Python Streaming job restricts pageviews to a subset of English wikipedia pages, filters out bad records, then sums hourly pageviews by keyed by Article-Date.  It emits output something like this:
...
A second MapReduce pass then maps the records by article name:
...
The records are merged at the reducers to generate a daily time series for each article in serialized JSON format (for easy manipulation in R & Python, or quick loading for the Web UI). 
...
TODO: examples from hive_daily_timelines.sql go here

We are only using Hive here for some simple joins and selects, but it has more powerful features for analytics.
...
Here are a few more quick queries to show how similar HiveQL is to SQL:
...
Along with powering the ranked lists of articles on the site, these trends are used to order autocomplete search results:
...
TODO: Brief description of running 2 Million Regressions with R
TODO: Mention Hive Sampling / Partition Bucket features
...
Show Python Streaming example, pulling input data from S3
Show regex on input bucket trick
Mention cron job to fetch and upload files to s3
