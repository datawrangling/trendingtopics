...
Launch an instance of the latest ec2onrails ami and note the returned instance address from ec2-describe-instances, it will be something like ec2-12-xx-xx-xx.z-1.compute-1.amazonaws.com. You can also launch and instance for each role (db, web, memcached, etc).  You probably want to assign a fixed "elastic-ip address":http://developer.amazonwebservices.com/connect/entry.jspa?externalID=1346 to each instance you use.
...
<pre>
ssh root@ec2-75-101-197-151.compute-1.amazonaws.com
cd mnt/app/current
RAILS_ENV=production rake db:develop
</pre>
