class PagesController < ApplicationController
  ...
  def index
    if params[:search]
      ...
      @pages = Page.paginate(:page => params[:page], :conditions => ["pages.id NOT IN (?) and id NOT IN (select page_id from featured_pages)", APP_CONFIG['blacklist']], :order => 'monthly_trend DESC', :per_page => APP_CONFIG['articles_per_page'])   
    end 
